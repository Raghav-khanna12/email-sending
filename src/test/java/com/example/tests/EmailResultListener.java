...
public class EmailResultListener implements ITestListener, ISuiteListener {
	...
	public void onFinish(ISuite suite) {
		...
		if (apiKey == null || apiKey.isBlank()) {
			Reporter.log("[EmailResultListener] SENDGRID_API_KEY not provided; skipping suite summary email", true);
	...
	public void onTestFailure(ITestResult result) {
		...
		String apiKey = System.getProperty("sendgrid.api.key", System.getenv().getOrDefault("SENDGRID_API_KEY", ""));
		if (apiKey == null || apiKey.isBlank()) {
			Reporter.log("[EmailResultListener] SENDGRID_API_KEY not provided; skipping failure email", true);
			return;
		}
		Set<String> recipients = new LinkedHashSet<>();
		if (toCsv != null && !toCsv.isBlank()) {
		...
		if (recipients.isEmpty()) return;
		String subject = "TestNG Failure: " + result.getMethod().getMethodName();
		String body = "<div style=\"font-family:Segoe UI,Arial,sans-serif;\">" +
				"<h3 style=\"color:#c0392b;margin:0 0 12px;\">" + subject + "</h3>" +
				"<p><strong>Class</strong>: " + result.getTestClass().getName() + "</p>" +
				"<p><strong>Throwable</strong>: " + (result.getThrowable() != null ? result.getThrowable().toString() : "(none)") + "</p>" +
				"</div>";
		try {
			new SendGridService(apiKey, fromEmail).sendEmail(recipients, subject, body);
		} catch (IOException e) {
			Reporter.log("Failed to send SendGrid failure email: " + e.getMessage(), true);
		}
	}
